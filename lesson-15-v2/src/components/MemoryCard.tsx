...
import { MapPin, Building2, UtensilsCrossed, StickyNote, Pencil, Trash2, X, Check } from 'lucide-react';
import { StarDisplay, StarRating } from './StarRating';
import type { FoodMemory } from '../lib/supabase';

interface MemoryCardProps {
  memory: FoodMemory;
  onEdit: (id: string, data: { city: string; place: string; dish: string; rating: number; note: string }) => void;
...
export function MemoryCard({ memory, onEdit, onDelete }: MemoryCardProps) {
  ...
  const handleSaveEdit = () => {
    ...
    onEdit(memory.id, { city: city.trim(), place: place.trim(), dish: dish.trim(), rating, note: note.trim() });
    setIsEditing(false);
  };
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              rows={2}
              className="w-full px-3.5 py-2.5 rounded-xl border border-stone-200 bg-white text-stone-800 focus:outline-none focus:ring-2 focus:ring-amber-400/40 focus:border-amber-400 transition-all duration-200 resize-none"
            />
          </div>
